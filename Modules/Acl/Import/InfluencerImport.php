...
class InfluencerImport implements ToModel, WithStartRow
{
    ...
    public function model(array $row)
    {
        ...
        if($checkArray)
        {
                            ...
                            ($row[4] != null && $row[26] != null) || ($row[5] != null && $row[31] != null) || ($row[6] != null && $row[35] != null))*/)
            {
                if($row[2])
                {
                    ...
                    $influencer = InfluencerFollowerPlatform::where('platform_id', $platform_id)->where('url', $row[2])
                        ->first();
                    if($influencer)
                    {
                        $dataArray = false;
                ...
                if($row[3])
                {
                    ...
                    $influencer = InfluencerFollowerPlatform::where('platform_id', $platform_id)->where('url', $row[3])
                        ->first();
                    if($influencer)
                    {
                        $dataArray = false;
                ...
                if($row[4])
                {
                    ...
                    $influencer = InfluencerFollowerPlatform::where('platform_id', $platform_id)->where('url', $row[4])
                        ->first();
                    if($influencer)
                    {
                        $dataArray = false;
                ...
                if($row[5])
                {
                    ...
                    $influencer = InfluencerFollowerPlatform::where('platform_id', $platform_id)->where('url', $row[5])
                        ->first();
                    if($influencer)
                    {
                        $dataArray = false;
                ...
                if($row[6])
                {
                    ...
                    $influencer = InfluencerFollowerPlatform::where('platform_id', $platform_id)->where('url', $row[6])
                        ->first();
                    if($influencer)
                    {
                        $dataArray = false;
        ...
        if($checkArray && $dataArray)
        {
            ...
            if($row[11])
            {
                $city = Location::where('name_en', $row[11])->orWhere('name_ar', $row[11])
