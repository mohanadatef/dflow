...
@section('title', getCustomTranslation('user').getCustomTranslation('import'))

@section('content')
<div class="container">
    <div class="card bg-light mt-3">
        <div class="card-header my-2 py-2">{{getCustomTranslation('user')}}{{getCustomTranslation('import')}}</div>
        <div class="card-body">
            ...
            @elseif(session()->has('bad'))
                ...
                @endphp
            @endif
            <form id="myForm" action="{{ route('user.import') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <input
                    type="file" name="file"
                    class="form-control"
...
@push('scripts')
<script src="{{asset('dashboard/assets/jquery/jquery-3.7.1.min.js')}}"></script>
...
<script>
    document.forms["myForm"].addEventListener("submit", async (event) => {
        ...
        setTimeout(async function () {
            const resp = await fetch('{{ route('user.import') }}' + '?check_status=1');
