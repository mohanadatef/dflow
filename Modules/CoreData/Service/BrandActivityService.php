...
use Modules\CoreData\Repositories\BrandActivityRepository;

class BrandActivityService extends BasicService
{
    protected $repo, $countryService, $influencerService, $platformService, $tagService,$companyService;
     ...
     * @return void
     */
    public function __construct(BrandActivityRepository $repository, LocationService $countryService,
        InfluencerService $influencerService,CompanyService $companyService,
        PlatformService $platformService, TagService $tagService)
    {
        ...
        $moreConditionForFirstLevel = [], $recursiveRel = [], $withRelations = [], $column = ['*'])
    {
        if(isset($request->search) && $request->search != null)
        {
            ...
            $moreConditionForFirstLevel = [
                'whereCustom' => [
                    'orWhere' => [
                        ['subject' => [$operator, $s]], ['brief' => [$operator, $s]], ['event_location' => [$operator, $s]]],
                ]];
            $recursiveRel = ['tag' =>
                [
                    'type' => 'whereHas',
    ...
    public function sponsorshipList(Request $request)
    {
        $request->merge(['active' => activeType()['as']]);
