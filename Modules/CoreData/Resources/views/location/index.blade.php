        ...
        <div class="card-header border-0 pt-6">
            ...
            <div class="card-title">
                ...
                <div class="d-flex align-items-center position-relative my-1">
                    ...
                    <input type="text"
                           id="search"
                           data-kt-location-table-filter="search" class="form-control form-control-solid w-250px ps-15"
                           placeholder="{{getCustomTranslation('search_location')}}"
            ...
            <div class="card-toolbar">
                @if(!$userType == 1)
                    ...
                    <!--begin::Menu 1-->
                    <form method="GET" id="filterDataForm" action="{{route('location.index')}}"
                        ...
                        <div class="px-7 py-5" data-kt-user-table-filter="form">
                            ...
                            <div class="mb-10">
                                <!--begin::Label-->
                                <label class="form-label fw-semibold">{{getCustomTranslation('country')}}:</label>
                                <!--end::Label-->
                                <!--begin::Input-->
                                <div>
                                    <select id="searchSelect" class="form-select form-select-solid" name="parent_id"
                                            data-kt-select2="true" data-placeholder="{{getCustomTranslation('select_option')}}">
                                        <option value="">{{getCustomTranslation('all')}}</option>
                                        @foreach($country as $value)
                                        <option {{ request()->country == $value->id ? "selected" : ""}} value="{{$value->id}}">{{$value->{'name_'.$lang} }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <!--end::Input-->
              ...
              {{getCustomTranslation('export')}}
                ...
                <!--begin::Toolbar-->
                <div class="d-flex justify-content-end" data-kt-location-table-select="base">
                    @can('create_location')
                    <!--begin::Add customer-->
                    <a href="{{ route('location.create') }}" class="btn btn-primary">{{getCustomTranslation('add')}} {{getCustomTranslation('location')}}</a>
                    <!--end::Add customer-->
                    @endcan
                </div>
                <!--end::Toolbar-->
                <!--begin::Group actions-->
                <div class="d-flex justify-content-end align-items-center" data-kt-location-table-select="selected">
                    <div class="fw-bold me-5">
                        {{-- <span class="me-2" data-kt-customer-table-select="selected_count"></span>Selected</div> --}}
                        @can('delete_location')
                        <button type="button" class="btn btn-danger ms-1 "
                                data-kt-location-table-select="delete_selected">{{getCustomTranslation('delete_selected')}}
                        </button>
                        @endcan
                    </div>
                    <!--end::Group actions-->
            ...
            <div class="card-body pt-0" id="data">
                <!--begin::Table-->
                @include('coredata::location.table')
            </div><!--end::Datatable-->
        </div>
    </div><!--end::Products-->
@endsection
@push('scripts')
    <script>
        let route = "{{ route('location.index') }}";
        ...
        let routeAll = "{{ route('location.index') }}?" + request_params_string;
        let csrfToken = "{{ csrf_token() }}";
        let deletePermission = {{permissionShow('delete_location') ? 1 : 0}};
        let updatePermission = {{permissionShow('update_location') ? 1 : 0}};
        let toggleActiveRoute = "{{ route('location.toggleActive') }}";
    </script>
    <script src="{{ asset('dashboard') }}/assets/js/location/list.js?v=12"></script>
    <script>
        ...
        $(document).ready(function () {
            $('#searchSelect').select2({
                placeholder: '{{getCustomTranslation('all')}}',
    ...
    <script>
        ...
        function exportData(lang) {
            var params = {
                lang: lang,
                parent_id: $("#searchSelect").val(),
            };
            location.href = "{{ route('location.export' )}}?" + jQuery.param(params);
        }
        ...
        $(document).on('change', '#limit', function () {
            $('#data').html('<div class="d-flex justify-content-center"><div style="height: 50px;width: 50px;margin: 50px;" class="spinner-border spinner-border-sm" role="status"><span class="sr-only">{{getCustomTranslation('loading')}}...</span></div></div>');
            limit = $(this).val();
