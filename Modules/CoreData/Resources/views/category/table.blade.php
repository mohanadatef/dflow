<table class="table align-middle table-row-dashed fs-6 gy-5"  id="kt_customers_table_new">
    ...
    <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
        ...
        <th class="min-w-125px">{{getCustomTranslation('group')}}</th>
    ...
    <tbody class="fw-semibold text-gray-600">
        @foreach ($categories as $category)
            <tr>
                ...
                <td>{{ $category->parents?->{'name_'.$lang} }}</td>
                <td>{{getCustomTranslation($category->group)}}</td>
                <td>
                    <div class="form-check form-switch form-switch-sm form-check-custom form-check-solid" >
                        <input class="form-check-input" type="checkbox" value="" name="notifications" {{ $category->active ? 'checked' : '' }} onclick="toggleActive({{ $category->id }})">
                        <label class="form-check-label" id="active-label-{{ $category->id }}"> {{ $category->active ? getCustomTranslation('active') : getCustomTranslation('inactive') }}</label>
                    </div>
                </td>
                    ...
                    <td class="text-end">
                        ...
                        <div
                            ...
                            <div class="menu-item px-3">
                                ...
                                <a href="#" data-kt-category-table-filter="delete_row"
...
{{ $categories->appends($_GET)->links('dashboard.layouts.pagination', ['paginator' => $categories,'perPage' =>Request::get('perPage') ?? $categories->perPage()]) }}
