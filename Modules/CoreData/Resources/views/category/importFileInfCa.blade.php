...
@section('content')
    <div class="container">
        <div class="card bg-light mt-3">
            <div class="card-header py-2 my-2">{{getCustomTranslation('category') }}{{getCustomTranslation('import')}}</div>
            <div class="card-body">
                ...
                @elseif(session()->has('bad'))
                    ...
                    @endphp
                @endif
                <form id="myForm" action="{{ route('category.import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <input
                        type="file" name="file"
                        class="form-control"
