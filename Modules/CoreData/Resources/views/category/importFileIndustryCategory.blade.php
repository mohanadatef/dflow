...
@section('title', getCustomTranslation('category').getCustomTranslation('import'))

@section('content')
    <div class="container">
        <div class="card bg-light mt-3">
            <div class="card-header py-2 my-2">{{getCustomTranslation('industry') }}{{getCustomTranslation('import')}}</div>
            <div class="card-body">
                ...
                @elseif(session()->has('bad'))
                    ...
                    @endphp
                @endif
                <form id="myForm" action="{{ route('category.import') }}?industry=1" method="POST" enctype="multipart/form-data">
                    @csrf
    ...
    <script>
        document.forms["myForm"].addEventListener("submit", async (event) => {
            ...
            setTimeout(async function () {
                const resp = await fetch('{{ route('category.import') }}' + '?check_status=1');
