    ...
    <div class="card mb-5 mb-xl-10">
        ...
        <div class="card-header border-0 cursor-pointer" role="button" data-bs-toggle="collapse"
            ...
            <div class="card-title m-0">
                <h3 class="fw-bold m-0">{{getCustomTranslation('category')}}</h3>
            </div>
            <!--end::Card title-->
        </div>
        <!--begin::Card header-->
        ...
        <div id="kt_account_settings_profile_details" class="collapse show">
            ...
            <form id="kt_category_create_form" class="form" method="post" action="{{route('category.store')}}">
                @csrf
                <!--begin::Card body-->
                <div class="card-body border-top p-9">
                    <div class="row">
                                ...
                                <div class="col-lg-8">
                                    <select id="groups" name="group" aria-label="{{getCustomTranslation('select_a_group')}}" data-control="select2"
                                            ...
                                            @foreach(groupType() as $value)
                                                <option {{Request::old('group') == $value ? 'selected' : "" }} value="{{$value}}">{{getCustomTranslation($value)}}</option>
                                            @endforeach
                        ...
                        <div class="col-lg-6">
                            <!--begin::Input group-->
                            <div class="row mb-6 {{Request::old('group') == 'industry_child' ?: "d-none" }}" id="parents">
                                <!--begin::Label-->
                                ...
                                <div class="col-lg-8">
                                    <select name="parent_id" aria-label="{{getCustomTranslation('select_a_parent')}}" data-control="select2"
                                        ...
                                        @foreach($category as $value)
                                            <option {{Request::old('parent_id') == $value->id ? 'selected' : "" }}  value="{{$value->id}}">{{$value->{'name_'.$lang} }}</option>
                                        @endforeach
                                    </select>
