...
@section('title', getCustomTranslation('platform') + getCustomTranslation('import'))

@section('content')
<div class="container">
    <div class="card bg-light mt-3">
        <div class="card-header py-2 my-2">{{getCustomTranslation('import')}} {{getCustomTranslation('platform')}}</div>
        <div class="card-body">
            ...
            @elseif(session()->has('bad'))
                ...
                @endphp
            @endif
            <form id="myForm" action="{{ route('platform.import') }}" method="POST" enctype="multipart/form-data">
                @csrf
...
<script>
    document.forms["myForm"].addEventListener("submit", async (event) => {
        ...
        setTimeout(async function () {
            const resp = await fetch('{{ route('platform.import') }}' + '?check_status=1');
