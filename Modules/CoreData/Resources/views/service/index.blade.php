    ...
    <div class="card card-flush">
        ...
        <div class="card-header border-0 pt-6">
          ...
          {{getCustomTranslation('export')}}
                ...
                <div class="d-flex justify-content-end" data-kt-customer-table-select="base">
                    @can('create_services')
                    <!--begin::Add customer-->
                    <a href="{{ route('service.create') }}" class="btn btn-primary">{{getCustomTranslation('add')}}
                        {{getCustomTranslation('service')}}</a>
                    <!--end::Add customer-->
                    @endcan
                </div>
                <!--end::Toolbar-->
        ...
        @push('scripts')
            ...
            <script>
                ...
                var routeAll = "{{ route('service.index',Request()->all()) }}";
                var csrfToken = "{{ csrf_token() }}";
                var deletePermission = {{permissionShow('delete_services') ? 1 : 0}};
                var updatePermission = {{permissionShow('update_services') ? 1 : 0}};
                var toggleActiveRoute = "{{ route('service.toggleActive') }}";
                var routeExport = "{{ route('service.export' )}}";
            </script>
            <script src="{{ asset('dashboard') }}/assets/js/service/list.js?v=1"></script>
    @endpush
