...
@if(count($dates))
    <table class="table align-middle table-row-dashed fs-6 gy-5"  id="kt_influencer_travel_table_new">
    <!--begin::Table head-->
    <thead>
    <!--begin::Table row-->
        ...
        <th class="w-10px pe-2">
            ...
            <div class="form-check form-check-sm form-check-custom form-check-solid me-3">
                <input class="form-check-input" type="checkbox" data-kt-check="true"
                       data-kt-check-target="#kt_influencer_travel_table_new .form-check-input" value="1"/>
            </div>

        </th>
        <th class="min-w-125px">{{getCustomTranslation('influencer')}}</th>
        <th class="min-w-125px">{{getCustomTranslation('country')}}</th>
        <th class="min-w-125px">{{getCustomTranslation('city')}}</th>
    ...
    <tbody class="fw-semibold text-gray-600">
        @foreach ($dates as $influencer_travel)
            <tr>
                <td>
                    <div class="form-check form-check-sm form-check-custom form-check-solid">
                        <input class="form-check-input" type="checkbox" name="item_check"
                               value="{{$influencer_travel->id}}"/>
                    </div>
                </td>
                <td>{{ $influencer_travel->influencer->{'name_'.$lang} }}</td>
                <td>{{ $influencer_travel->country->{'name_'.$lang} }}</td>
                <td>{{ $influencer_travel->city->{'name_'.$lang} }}</td>
                <td>{{ date('Y-m-d', strtotime($influencer_travel->date_from))}}</td>
                <td>{{ date('Y-m-d', strtotime($influencer_travel->date_to))}}</td>
                @can('update_influencer_travel')
                    <td class="text-end">
                        ...
                        <div
                            ...
                            <div class="menu-item px-3">
                                @can('update_influencer_travel')
                                <a href="{{route('influencer_travel.edit',$influencer_travel->id)}}"
                            ...
                            <div class="menu-item px-3">
                                ...
                                <a href="#" data-kt-influencer_travel-table-filter="delete_row"
