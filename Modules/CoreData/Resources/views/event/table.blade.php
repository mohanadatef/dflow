...
@if(count($dates))
    <table class="table align-middle table-row-dashed fs-6 gy-5"  id="kt_event_table_new">
    <!--begin::Table head-->
    <thead>
    <!--begin::Table row-->
        ...
        <th class="w-10px pe-2">
            ...
            <div class="form-check form-check-sm form-check-custom form-check-solid me-3">
                <input class="form-check-input" type="checkbox" data-kt-check="true"
                       data-kt-check-target="#kt_event_table_new .form-check-input" value="1"/>
            </div>
    ...
    <tbody class="fw-semibold text-gray-600">
        @foreach ($dates as $event)
            <tr>
                ...
                <td>{{ $event->subject }}</td>
                <td>{{implode(',',$event->influencer->pluck('name_'.$lang)->toArray())}}</td>
                <td>{{implode(',',$event->category->pluck('name_'.$lang)->toArray())}}</td>
                <td>{{ $event->country->{'name_'.$lang} }}</td>
                <td>{{ $event->tag->{'name_'.$lang} }}</td>
                ...
                @can('update_event')
                    <td class="text-end">
                        ...
                        <div
                            ...
                            <div class="menu-item px-3">
                                @can('delete_event')
                                <a href="#" data-kt-event-table-filter="delete_row"
