...
@section('title', getCustomTranslation('brand_activity'))
...
@section('content')
    ...
    <div class="card card-flush">
        ...
        <div class="card-header border-0 pt-6">
            ...
            <div class="card-title">
                ...
                <div class="d-flex align-items-center position-relative my-1">
                    ...
                    <input type="text"
                           id="search"
                           data-kt-brand_activity-table-filter="search" class="form-control form-control-solid w-250px ps-15"
                           placeholder="{{getCustomTranslation('search')." ".getCustomTranslation('brand_activity')}}"
            ...
            <div class="card-toolbar">
                ...
                <!--begin::Menu 1-->
                <form method="GET" id="filterDataForm" action="{{route('brand_activity.index')}}"
                    ...
                    <div class="px-7 py-5" data-kt-user-table-filter="form">
                        ...
                        <div class="mb-10">
                            <label class="form-label fs-6 fw-semibold">{{getCustomTranslation('sponsorship')}}:</label>
                            <!--begin::Input-->
                            <select id="sponsorship" name="sponsorship[]"
                                    class="form-select form-select-solid form-select-lg fw-semibold"
                                    data-mce-placeholder="" multiple>
                                @if( request('sponsorship'))
                                    @foreach($sponsorship_datas as $sponsorship_data)
                                        <option value="{{$sponsorship_data->id}}"
                                                @if(in_array($sponsorship_data->id , request('sponsorship') ?: [])) selected @endif>{{$sponsorship_data->{'name_'.$lang} }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
                        ...
                        <div class="d-flex justify-content-end">
                            <a href="{{ route('brand_activity.index') }}" id="reset_filter"
                ...
                <!--begin::Toolbar-->
                <div class="d-flex justify-content-end" data-kt-brand_activity-table-select="base">
                    @can('create_brand_activity')
                        <!--begin::Add customer-->
                        <a href="{{ route('brand_activity.create') }}"
                           class="btn btn-primary">{{getCustomTranslation('add')}} {{getCustomTranslation('brand_activity')}}</a>
                        <!--end::Add customer-->
                    @endcan
                </div>
                <!--end::Toolbar-->
                <!--begin::Group actions-->
                <div class="d-flex justify-content-end align-items-center" data-kt-brand_activity-table-select="selected">
                    <div class="fw-bold me-5">
                        {{-- <span class="me-2" data-kt-customer-table-select="selected_count"></span>Selected</div> --}}
                        @can('delete_brand_activity')
                            <button type="button" class="btn btn-danger ms-1 "
                                    data-kt-brand_activity-table-select="delete_selected">{{getCustomTranslation('delete_selected')}}
                            </button>
                        @endcan
                    </div>
                    <!--end::Group actions-->
        ...
        <div class="card-body pt-0" id="data">
            <!--begin::Table-->
            @include('coredata::brand_activity.table')
        </div><!--end::Datatable-->
    </div><!--end::Products-->
@endsection
@push('scripts')
    <script>
        let route = "{{ route('brand_activity.index') }}";
        var searchVal = $("#search").val().trim();//?search="+searchVal+"&"+ "perPage=" + limit +"&"
        let csrfToken = "{{ csrf_token() }}";
        let deletePermission = {{permissionShow('delete_brand_activity') ? 1 : 0}};
        let updatePermission = {{permissionShow('update_brand_activity') ? 1 : 0}};
        limit = "{{Request::get('perPage') ?? $dates->perPage()}}";
    </script>
    <script src="{{ asset('dashboard') }}/assets/js/brand_activity/list.js"></script>
    <script>
        ...
        function exportData(lang) {
            ...
            location.href = "{{ route('brand_activity.export' )}}?" + jQuery.param(params);
        }
        ...
        $('#sponsorship').select2({
            ...
            delay: 900,
            placeholder: "{{getCustomTranslation('select_a_sponsorship')}}...",
            ajax: {
                cacheDataSource: [],
