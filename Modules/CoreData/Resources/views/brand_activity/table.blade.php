...
@if(count($dates))
    <table class="table align-middle table-row-dashed fs-6 gy-5"  id="kt_brand_activity_table_new">
    <!--begin::Table head-->
    <thead>
    <!--begin::Table row-->
        ...
        <th class="w-10px pe-2">
            ...
            <div class="form-check form-check-sm form-check-custom form-check-solid me-3">
                <input class="form-check-input" type="checkbox" data-kt-check="true"
                       data-kt-check-target="#kt_brand_activity_table_new .form-check-input" value="1"/>
            </div>

        </th>
        <th class="min-w-125px">{{getCustomTranslation('subject')}}</th>
        <th class="min-w-125px">{{getCustomTranslation('influencer')}}</th>
        <th class="min-w-125px">{{getCustomTranslation('sponsorship')}}</th>
    ...
    <tbody class="fw-semibold text-gray-600">
        @foreach ($dates as $brand_activity)
            <tr>
                <td>
                    <div class="form-check form-check-sm form-check-custom form-check-solid">
                        <input class="form-check-input" type="checkbox" name="item_check"
                               value="{{$brand_activity->id}}"/>
                    </div>
                </td>
                <td>{{ $brand_activity->subject }}</td>
                <td>{{implode(',',$brand_activity->influencer->pluck('name_'.$lang)->toArray())}}</td>
                <td>{{implode(',',$brand_activity->sponsorship->pluck('name_'.$lang)->toArray())}}</td>
                <td>{{implode(',',$brand_activity->platform->pluck('name_'.$lang)->toArray())}}</td>
                <td>{{ $brand_activity->country->{'name_'.$lang} }}</td>
                <td>{{ $brand_activity->tag->{'name_'.$lang} }}</td>
                <td>{{$brand_activity->date_from}}</td>
                <td>{{ $brand_activity->date_to}}</td>
                @can('update_brand_activity')
                    <td class="text-end">
                        ...
                        <div
                            ...
                            <div class="menu-item px-3">
                                @can('update_brand_activity')
                                <a href="{{route('brand_activity.edit',$brand_activity->id)}}"
                            ...
                            <div class="menu-item px-3">
                                ...
                                <a href="#" data-kt-brand_activity-table-filter="delete_row"
