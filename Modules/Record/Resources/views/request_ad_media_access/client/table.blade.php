    ...
    @if(count($datas))
        ...
        <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_users_table">
            ...
            <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                <th class="min-w-125px">{{getCustomTranslation('id_request')}}</th>
                <th class="min-w-125px">{{getCustomTranslation('ad_Date')}}</th>
                <th class="min-w-125px">{{getCustomTranslation('ad_Platform')}}</th>
                <th class="min-w-125px">{{getCustomTranslation('ad_Company')}}</th>
                <th class="min-w-125px">{{getCustomTranslation('ad_influencer')}}</th>
                <th class="min-w-125px">{{getCustomTranslation('request_create_at')}}</th>
            ...
            @foreach($datas as $data)
                <tr>
                    ...
                    <td  id="status-{{$data->ad_record_id}}">
                    ...
                    <td class="text-end">
                        ...
                        <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4"
                             data-kt-menu="true">
                            @if($data->status == 1)
                            <div class="menu-item px-3"  id="action-{{$data->ad_record_id}}" >
                                <a href="#" onclick="cancellation({{$data->ad_record_id}})"
                                   class="menu-link px-3">{{getCustomTranslation('cancel')}}</a>
                            </div>
                            @endif
