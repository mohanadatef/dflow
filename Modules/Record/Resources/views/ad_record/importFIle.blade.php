...
@section('title',getCustomTranslation('ad_records_import') )

@section('content')
<div class="container">

    <div class="card bg-light mt-3">
        <div class="card-header py-2 my-2">{{getCustomTranslation('ad_records_import')}}</div>
        <div class="card-body">
            @if(session()->has('message'))
                ...
                @endphp
            @endif
            <form id="myForm" action="{{ route('ad_record.import') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <a href="{{ route('ad_record.download') }}" class="btn btn-danger download_link"
                   @if($import_in_progress == 1) style="display: none;" @endif
                   title="{{getCustomTranslation('download_rows_with_missing_data')}}"
                   data-toggle="tooltip"
                >
...
@push('scripts')
    ...
    <script>
        ...
        RepeatFun();
        @endif
        let status_url = "{{ route('ad_record.status') }}";
    </script>
@endpush
