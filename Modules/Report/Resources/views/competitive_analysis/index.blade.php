...
@section('title',  getCustomTranslation('competitive_analysis'))
        ...
        <div class="row" style="margin-bottom:50px;">
            ...
            @else
                        ...
                        <div class="row">
                            ...
                            <div class="d-flex align-items-center position-relative my-1">
                                <select id="search_company_direct" name="company_id"
                                        class="form-select form-select-solid form-select-lg fw-semibold"
                                        data-mce-placeholder="{{getCustomTranslation('select_company_direct')}}"
                                >
                                    <option disabled selected>{{getCustomTranslation('select_company_direct')}}</option>
                                    @foreach($companies_direct as $compan)
                                        <option
                                            value="{{$compan->id}}">{{$compan->{'name_'.$lang} }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="row">
                            <div class="d-flex align-items-center position-relative my-1">
                                <select id="search_company_indirect" name="company_id"
                                        class="form-select form-select-solid form-select-lg fw-semibold"
                                        data-mce-placeholder="{{getCustomTranslation('select_company_indirect')}}">
                                    <option disabled selected>{{getCustomTranslation('select_company_indirect')}}</option>
                ...
                <div class="row g-5 g-xl-10">
                    ...
                    <div id="main-content">
                        @include('report::competitive_analysis.main')
                ...
                @push('scripts')
                    ...
                    <script>
                        ...
                        let path = "{{ route('reports.search_companies') }}";
                        ...
                        function loadMainContent(id, ranges) {
                            $.get({
                                url: "{{route('reports.competitive_analysis')}}",
                                data: {
                                    company: id,
                                    ranges: ranges,
                                },
